...
transports:
  - type: "http"
    config:
      ...
      # TLS/HTTPS settings (nested configuration)
      tls:
        enabled: true
        ...
        min_version: "TLS1.2"           # TLS1.2 or TLS1.3
...
For development and testing, you can auto-generate self-signed certificates:
  ...
  - type: "http"
    config:
      ...
      tls:
        ...
        common_name: "localhost"      # Certificate common name
        san_names:                    # Subject Alternative Names
          - "mynetwork.local"
          - "192.168.1.100"
```
...
2. Generate server certificate (`server.crt`) and key (`server.key`)
3. Include localhost, 127.0.0.1, and ::1 by default
4. Store certificates in the specified `cert_dir`

### Claude Code / Claude Desktop Integration

Claude Code requires HTTPS for remote MCP connections. To expose your network to Claude Code:
...
```json
{
  "mcpServers": {
    "my-network": {
      "url": "https://your-domain.com:8443/mcp"
    }
  }
}
```
...
Use a reverse proxy with automatic TLS:
```
# Caddyfile
your-domain.com {
...
Then your MCP endpoint is available at `https://your-domain.com/mcp`.
  ...
  - type: "grpc"
    config:
      ...
      tls:
        ...
        cert_file: "/etc/ssl/certs/server.crt"
        key_file: "/etc/ssl/private/server.key"
        require_client_cert: true  # Require client certificates (mTLS)
```
