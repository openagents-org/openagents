  ...
  useEffect(() => {
    ...
    const timers = BEAT_TIMES.map((t, i) => {
      ...
      return setTimeout(() => {
        ...
        setTimeout(() => {
          setBeat(i);
          requestAnimationFrame(() => setDissolving(false));
        }, DISSOLVE);
      }, t);
    });
  ...
  return (
    <div className="fixed inset-0 z-[100] flex flex-col bg-white overflow-hidden" style={{ opacity: dismissed ? 0 : 1, transition: 'opacity 600ms ease' }}>
      ...
      <style>{`
        ...
        @keyframes onb-drift-0 { 0% { transform: translate(0,0) } 33% { transform: translate(5px,-4px) } 66% { transform: translate(-3px,5px) } 100% { transform: translate(0,0) } }
        @keyframes onb-drift-1 { 0% { transform: translate(0,0) } 33% { transform: translate(-5px,6px) } 66% { transform: translate(4px,-3px) } 100% { transform: translate(0,0) } }
        @keyframes onb-drift-2 { 0% { transform: translate(0,0) } 33% { transform: translate(4px,5px) } 66% { transform: translate(-6px,-2px) } 100% { transform: translate(0,0) } }
      `}</style>
    </div>
  );
}
...
function SceneConnect({ agentCount }: { agentCount: number }) {
  ...
  return (
    <div className="relative flex flex-col items-center justify-center gap-8">
      {/* Connection network */}
      <div className="relative overflow-hidden" style={{ width: 280, height: 240 }}>
...
function SceneCollaborate() {
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-12 px-8">
      ...
      <div className="relative flex items-center gap-16 sm:gap-24 overflow-visible">
        {AGENTS.map((agent, i) => (
          <div
            key={agent.name}
          ...
          >
            <div className="size-16 sm:size-20 flex items-center justify-center overflow-hidden">
...
function SceneShare() {
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-12 px-8">
      ...
      <div className="relative overflow-hidden" style={{ width: 300, height: 200 }}>
        {/* Document shapes */}
        {docs.map((doc, i) => (
          <div
            key={i}
...
function SceneAutomate() {
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-12 px-8">
      {/* Orbital visual */}
      <div className="relative overflow-hidden" style={{ width: orbitRadius * 2 + 120, height: orbitRadius * 2 + 120 }}>
        {/* Orbit ring */}
        <div
