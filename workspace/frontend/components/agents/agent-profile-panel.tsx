...
export function AgentProfilePanel({ docked = false }: { docked?: boolean } = {}) {
  ...
  const handleRemoveCloudAgent = useCallback(async () => {
    ...
    try {
      ...
      toast.success(t('agents.removedCloud', { agent: agent.agentName }));
      setSelectedAgentName(null);
      refreshWorkspace();
    } catch {
      toast.error(t('agents.removeCloudFailed'));
    }
  }, [agent, setSelectedAgentName, refreshWorkspace]);

  // Remove a node/local agent from the workspace (works online or offline).
  ...
  }, [agent, confirm, t, setSelectedAgentName, refreshWorkspace]);
  ...
  const handleUpdateApiKey = useCallback(async () => {
    ...
    try {
      ...
      toast.success(t('agents.apiKeyUpdated'));
      setEditingKey(false);
      setNewApiKey('');
      workspaceApi.listCloudAgents().then((configs) => {
        setCloudConfig(configs.find((c) => c.agentName === agent.agentName) || null);
      ...
      toast.error(t('agents.apiKeyFailed'));
    } finally {
      setSavingKey(false);
    }
  }, [agent, newApiKey]);
  ...
  const handleGenerateDescription = useCallback(async () => {
    ...
    try {
      ...
      if (suggestion) {
        ...
        setDescDirty(true);
        toast.success(t('agents.autoGenerated'));
      } else {
        toast.error(t('agents.autoGenerateEmpty'));
      }
    } catch {
      toast.error(t('agents.autoGenerateFailed'));
    } finally {
  ...
  const infoItems = isCloud
    ...
    : [
        ...
        { icon: <Globe className="size-3.5" />, label: t('agents.fieldServer'), value: agent.serverHost || '—' },
        { icon: <Folder className="size-3.5" />, label: t('agents.fieldFolder'), value: agent.workingDir || '—' },
        { icon: <UserRoundCog className="size-3.5" />, label: t('agents.fieldAgentId'), value: `openagents:${agent.agentName}`, copyable: true },
      ];
    ...
    <>
      ...
      <div className={cn(
        ...
        <div className="flex-1 overflow-y-auto px-3.5 space-y-3">
          ...
          <div className="rounded-lg border overflow-hidden">
            <div className="px-3.5 py-2.5 border-b flex items-center justify-between gap-2">
              ...
              <button
                ...
                title={t('agents.autoGenerateHint')}
              >
                {generatingDesc
                  ? <RefreshCw className="size-3 animate-spin" />
                  : <Sparkles className="size-3 text-amber-500" />}
                {generatingDesc ? 'Generating…' : t('agents.autoGenerate')}
              </button>
            </div>
            <div className="p-3">
              <textarea
                  ...
                  <div className="flex-1 min-w-0 flex items-start gap-1">
                    ...
                    {item.copyable && (
                      <button
                        ...
                        title={t('agents.copyField', { field: item.label })}
          ...
          {/* Model — picker fed by the agent/provider catalog when it matches
              the configured endpoint; otherwise accepts a custom model id.
              ...
              <div className="p-3 space-y-3">
                ...
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-[11px] text-muted-foreground">{t('agents.fieldApiKey')}</span>
                    {!editingKey && (
                      <button
                ...
                {cloudConfig.systemPrompt && (
                  <div>
                    <span className="text-[11px] text-muted-foreground">{t('agents.systemPrompt')}</span>
          ...
          {(() => {
            ...
            return (
              <div className="rounded-lg border overflow-hidden">
                <div className="px-3.5 py-2.5 border-b flex items-center gap-1.5">
                  <Sparkles className="size-3 text-amber-500" />
                  <span className="text-xs font-medium">{t('agents.installedSkills')}</span>
          ...
          {/* Danger zone — remove this agent from the workspace (node/local
              agents; cloud agents use the footer Remove button). */}
          {!isCloud && (
            <div className="rounded-lg border border-red-200 dark:border-red-900/50 overflow-hidden">
              <div className="px-3.5 py-2.5 border-b border-red-200 dark:border-red-900/50">
                <span className="text-xs font-medium text-red-600 dark:text-red-400">{t('agents.dangerZone')}</span>
              </div>
              <div className="p-3 flex items-center justify-between gap-3">
                <p className="text-[11px] text-muted-foreground leading-relaxed">{t('agents.removeAgentHint')}</p>
                <button
                  onClick={handleRemoveAgent}
                  className="shrink-0 flex items-center gap-1.5 px-3 py-2 text-xs font-medium rounded-lg border border-red-200 dark:border-red-900/50 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
                >
