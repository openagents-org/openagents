...
function HeaderSlot({ id, children }: { id: string; children: React.ReactNode }) {
...
export function DetailHeader({
  ...
  return (
    <div className="flex h-12 shrink-0 items-center gap-2 border-b border-border px-2 lg:px-4">
      <div className="flex min-w-0 flex-1 items-center gap-2 lg:gap-3">{title}</div>
      <div className="flex shrink-0 items-center gap-1 lg:gap-1.5">{children}</div>
    </div>
  );
}
...
export function AppHeader() {
  ...
  if (viewMode === 'threads' || viewMode === 'routines') {
    ...
    const name = files.find((f) => f.id === selectedFileId)?.filename || currentFilePath;
    title = (
