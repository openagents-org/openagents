...
same answer twice. This table absorbs that: a reply naming the message it
answers can only land once per (workspace, agent, answered message).
...
def upgrade() -> None:
    ...
    if not _has_table(inspector, "message_replies"):
        op.create_table(
            ...
            sa.UniqueConstraint("workspace_id", "source", "in_reply_to", name="uq_message_reply_once"),
        )
