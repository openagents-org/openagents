...
# ---------------------------------------------------------------------------
# SWE-bench evaluation jobs
# ---------------------------------------------------------------------------

class EvaluationJob(Base):
    """A single SWE-bench evaluation task.

    This is a *benchmark* job, not a chat message and not an agent. One row
    drives one instance end-to-end: prepare an isolated working dir at the
    instance base commit, hand the issue to a connected coding agent, collect
    the resulting git patch, run the official SWE-bench Docker harness, and
    record the verdict. Long-running work happens off the request path in the
    background evaluation worker (see app.swebench.service).
    """
    __tablename__ = "evaluation_jobs"

    id = Column(Text, primary_key=True, default=_uuid)
    workspace_id = Column(UUID(as_uuid=False), ForeignKey("workspaces.id", ondelete="CASCADE"), nullable=False)
    # Dedicated channel where the agent receives the task and posts progress.
    channel_name = Column(Text, nullable=False)
    created_by = Column(Text, nullable=False)            # "human:..." or "openagents:..."

    # ── Task definition ──
    dataset = Column(Text, nullable=False)               # logical key, e.g. "swe_bench_lite"
    split = Column(Text, nullable=False, default="test")
    instance_id = Column(Text, nullable=False)
    repo = Column(Text, nullable=True)                   # "owner/name" (from the instance)
    base_commit = Column(Text, nullable=True)
    selected_agent = Column(Text, nullable=False)        # bare agent_name of the connected coding agent

    # ── Lifecycle ──
    # queued | preparing | agent_running | patch_collected | evaluating |
    # completed | failed | timeout | cancelled | error | integrity_rejected
    status = Column(Text, nullable=False, default="queued")
    # Fine-grained terminal outcome (set once the harness verdict is known):
    # resolved | unresolved | no_patch | patch_invalid | tests_failed |
    # error | timeout | cancelled | integrity_rejected
    outcome = Column(Text, nullable=True)
    resolved = Column(Boolean, nullable=True)            # harness "resolved" verdict (null until evaluated)
    # Coarse machine-readable failure bucket for the UI / debugging:
    # docker_unavailable | image_pull_failed | env_build_failed | patch_invalid
    # | tests_failed | harness_error | timeout | cancelled | no_patch
    # | agent_error | precheck_failed | internal_error | integrity_error
    # | integrity_rejected
    error_category = Column(Text, nullable=True)
    error_reason = Column(Text, nullable=True)           # human-readable, redacted (no secrets/prompts)
    cancel_requested = Column(Boolean, nullable=False, default=False)
    # Benchmark integrity controls/results.
    integrity_mode = Column(Text, nullable=False, default="strict")   # strict | debug
    integrity_risk = Column(Boolean, nullable=False, default=False)   # debug-mode test/infra edits ran anyway
    # Reproducibility snapshot: swebench/python/docker versions, os/arch, redacted command.
    environment = Column(JSONB, nullable=True)

    # ── Artifacts & run metadata ──
    patch_file_id = Column(Text, nullable=True)          # FileRecord id of the collected patch
    log_file_id = Column(Text, nullable=True)            # FileRecord id of the raw harness log bundle
    run_id = Column(Text, nullable=True)                 # harness --run_id (used to find/cleanup containers)
    work_dir = Column(Text, nullable=True)               # absolute path of the per-instance working dir
    docker_info = Column(JSONB, nullable=True)           # {namespace, image, container, exit_code, ...}
    report = Column(JSONB, nullable=True)                # parsed per-instance harness report (tests_status etc.)

    # ── Timing ──
    started_at = Column(DateTime(timezone=True), nullable=True)         # work began (preparing)
    agent_started_at = Column(DateTime(timezone=True), nullable=True)
    eval_started_at = Column(DateTime(timezone=True), nullable=True)
    finished_at = Column(DateTime(timezone=True), nullable=True)
    created_at = Column(DateTime(timezone=True), default=_now, server_default=text("NOW()"))

    __table_args__ = (
        Index("idx_evaluation_jobs_workspace_status", "workspace_id", "status"),
        Index("idx_evaluation_jobs_workspace_created", "workspace_id", "created_at"),
    )
