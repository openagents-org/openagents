...
class GeminiAdapter extends BaseAdapter {
  ...
  constructor(opts) {
    ...
    // Runs the user stopped: their exit is neither a failure to report nor a
    // reason to retry from a fresh session.
    this._userStopped = new WeakSet();
      ...
      try {
        ...
        _shouldRetry = await new Promise((resolve, reject) => {
          ...
          proc.on('exit', async (code, signal) => {
            ...
            // Stopped by the user: the stop control already told the channel.
            if (this._userStopped.has(proc)) {
              resolve(false);
              return;
            }
