...
function makeAdapter({ sessions = {} } = {}) {
  ...
  a.posted = { response: [], error: [], status: [], thinking: [], notices: [] };
  a.client = {
    getSession: async () => ({}),
    updateSession: async () => {},
    // The stop notice goes out as a plain message, straight through the client.
    sendMessage: async (_w, _c, _t, content) => { a.posted.notices.push(content); },
  };
...
describe('GeminiAdapter — retrying without resume', () => {
  ...
  it('a run the user stopped is neither retried nor reported as a failure', async () => {
    ...
    await a._handleUserStop('chan-1');
    await handled;

    assert.equal(spawns.length, 1);
    assert.deepEqual(a.posted.error, []);
    assert.deepEqual(a.posted.response, []);
    assert.ok(a.posted.notices.some((t) => /stopped by user/i.test(t)));
