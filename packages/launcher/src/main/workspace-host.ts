...
export class WorkspaceHost {
  ...
  show(target: string, bounds: ViewBounds, token?: string | null): void {
    ...
    slog(
      `[workspace-view] show target=${target || "(home)"} local=${local} url=${url}`,
    )
    if (url !== this._url) {
      this._url = url
    ...
    slog(
      `[workspace-view] bounds x=${Math.round(bounds.x)} y=${Math.round(bounds.y)} w=${Math.round(bounds.width)} h=${Math.round(bounds.height)}`,
    )
  }
  ...
  private _ensureView(): WebContentsView {
    ...
    view.webContents.on("did-finish-load", () =>
      slog(`[workspace-view] loaded ${view.webContents.getURL()}`),
    )
