...
/**
 * Sign in with an email and password held in this Firebase project.
 *
 * Rarely the right call, and never the first one. The website keeps its email
 * accounts on the account service (endpoint.openagents.org), so a password it
 * accepts comes back INVALID_LOGIN_CREDENTIALS here; what Firebase still holds
 * is the Google/GitHub/Apple identities, which have no password to send. This
 * exists for a deployment with no account service in front of it — and, like
 * the rest of this file, cannot work where Google is blocked.
 */
export async function signInWithPassword(
  ...
  return tokensFrom(
    await post(`${IDENTITY}/accounts:signInWithPassword`, {
      email,
      password,
      returnSecureToken: true,
    }),
