...
/**
 * Three steps, matching Button: `default` for forms, `sm` for toolbars, `xs`
 * for controls riding inside dense content (chips, inline settings rows).
 */
export type SelectSize = "xs" | "sm" | "default"
...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      data-size={size}
      className={cn(
        "flex w-fit items-center justify-between gap-2 rounded-md border border-input bg-transparent whitespace-nowrap shadow-xs transition-[color,box-shadow] outline-none",
        ...
        "data-[size=xs]:h-6 data-[size=xs]:gap-1 data-[size=xs]:px-2 data-[size=xs]:text-xs",
        // Same affordance as `Button variant="outline"` — these sit side by
        // side in every toolbar and used to be the only inert one.
...
function SelectContent({
  className,
  children,
  // Popper, aligned to the trigger's start edge — the same anchoring as every
  // other overlay in the app (dropdown menu, popover). `item-aligned` covers
  // the trigger like a native macOS menu and reads as a different family.
  position = "popper",
  align = "start",
  sideOffset = 4,
  size = "default",
  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Content
        ...
        data-size={size}
        className={cn(
          "group/select-content relative z-50 max-h-(--radix-select-content-available-height) min-w-32 origin-(--radix-select-content-transform-origin) overflow-x-hidden overflow-y-auto rounded-md border bg-popover text-popover-foreground shadow-md data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95",
          // At least as wide as its trigger, so a list opened from a full-width
          // field lines up with it instead of hanging off one corner. Bracket
          // form so tailwind-merge drops the `min-w-32` above instead of
          // keeping both (it doesn't read the v4 `min-w-(--var)` shorthand).
          position === "popper" &&
      ...
      >
        ...
        <SelectPrimitive.Viewport
          className={cn(
            "p-1",
            // Match the trigger's width so the list lines up under it. No
            // fixed height: upstream pins this to the trigger height, which
            // clips every list longer than one row.
