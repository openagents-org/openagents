    ...
    <div className="flex items-center gap-3 border-b border-border bg-muted/40 px-4 py-2 text-xs">
      ...
      <p className="min-w-0 flex-1 text-muted-foreground">
        {canAuthorize
          ...
          : t("account.device.adminOnly", { section: t("nav.items.workspaces.label") })}
      </p>
      {canAuthorize ? (
        <Button size="sm" onClick={() => void authorize()} disabled={busy}>
          {busy ? <Spinner className="size-3.5" /> : <ShieldCheck className="size-3.5" />}
          {busy ? t("account.device.authorizing") : t("account.device.authorize")}
        </Button>
      ) : (
