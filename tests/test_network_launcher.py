...
"""
    ...
    def test_persistence_manager_import(self):
        """Test that PersistenceManager can be imported and instantiated."""
        from openagents.core.persistence import PersistenceManager
        import tempfile
        
        # Test that we can create a persistence manager
        temp_dir = tempfile.mkdtemp()
        pm = PersistenceManager(temp_dir)
        assert pm.workspace_dir.exists()
        assert pm.global_dir.exists()
        assert pm.mods_dir.exists()
