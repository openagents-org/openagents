...
class BasicTestAgentAdapter(BaseModAdapter):
    ...
    def shutdown(self) -> bool:
        ...
        """
        logger.info(f"Basic Test agent adapter shutting down. Processed {self.event_count} events.")
        return True
    ...
    async def process_incoming_event(self, event: Event) -> Optional[Event]:
        ...
        if self.log_all_events:
            logger.info(f"Agent {self.agent_id} processing incoming event #{self.event_count}: {event.event_name}")
    ...
    async def process_outgoing_event(self, event: Event) -> Optional[Event]:
        ...
        # Log outgoing event
        if self.log_all_events:
            logger.info(f"Agent {self.agent_id} processing outgoing event: {event.event_name}")
...
>>>>>>> feature/krane-development
    ...
    async def _handle_send_test_message(self, parameters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.connector:
                ...
                return {"success": True, "message": f"Test message sent to {target_desc}"}
            else:
                return {"success": False, "error": "No connector available"}
        except Exception as e:
    ...
    def register_message_handler(self, event_name: str, handler: MessageHandler) -> None:
