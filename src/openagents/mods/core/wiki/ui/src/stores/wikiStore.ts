...
import { WikiModService } from "../lib/context";
...
interface WikiState {
    ...
    service: WikiModService | null;

    // Actions
    setService: (service: WikiModService | null) => void;
...
export const useWikiStore = create<WikiState>((set, get) => {
    // Helper function to get service from store or window context
    const getServiceFromStoreOrContext = (): WikiModService | null => {
        const { service } = get();

        // If service is already set in store, return it
        if (service) {
            console.log("WikiStore: Service found in store");
            return service;
        }

        // Try to get from window context
        // @ts-ignore - Access Studio's context through window
        const studioContext = (window as any).__OPENAGENTS_CONTEXT__;

        console.log("WikiStore: Checking window.__OPENAGENTS_CONTEXT__", {
            exists: !!studioContext,
            hasConnector: !!studioContext?.connector,
            isConnected: studioContext?.isConnected,
            connectorType: studioContext?.connector ? typeof studioContext.connector : 'none',
        });

        if (studioContext?.connector) {
            const connector = studioContext.connector;
            
            // Verify connector has required methods
            if (typeof connector.sendEvent !== 'function') {
                console.error("WikiStore: Connector does not have sendEvent method", connector);
                return null;
            }

            // Create service adapter
            const adapterService: WikiModService = {
                sendEvent: async (event: {
                    event_name: string;
                    destination_id: string;
                    payload: any;
                }) => {
                    console.log("WikiStore: Sending event via adapter service", event.event_name);
            ...
            set({ service: adapterService });
            console.log("WikiStore: Service adapter created and set to store");
            return adapterService;
        } else {
            console.warn("WikiStore: window.__OPENAGENTS_CONTEXT__ not available or connector missing", {
                hasContext: !!studioContext,
                contextKeys: studioContext ? Object.keys(studioContext) : [],
            });
        }
    ...
    return {
        ...
        setService: (service: WikiModService | null) => {
            set({ service });
        },

        loadPages: async () => {
            const service = getServiceFromStoreOrContext();
            if (!service) {
        ...
        createPage: async (pagePath: string, title: string, content: string) => {
            ...
            console.log("WikiStore: Creating page with data:", service ? "service available" : "service unavailable", pagePath, title, content);
            if (!service || !pagePath.trim() || !title.trim() || !content.trim()) {
                console.error("WikiStore: Cannot create page - missing service or required fields", {
                    hasService: !!service,
            ...
            try {
                ...
                if (response.success) {
                    ...
                    const newPage: WikiPage = {
                        ...
                        title: title.trim(),
                        wiki_content: content.trim(),
                        creator_id: service.getAgentId() || "unknown",
        ...
        editPage: async (pagePath: string, content: string) => {
            const service = getServiceFromStoreOrContext();
            const { loadPage, loadPages } = get();
            if (!service || !content.trim()) {
                console.warn("WikiStore: Cannot edit page - service not available or content empty");
                return false;
            }
        ...
        proposeEdit: async (pagePath: string, content: string, rationale: string) => {
            const service = getServiceFromStoreOrContext();
            const { loadProposals } = get();
            if (!service || !content.trim() || !rationale.trim()) {
                console.warn("WikiStore: Cannot propose edit - service not available or required fields missing");
                return false;
            }
        ...
        resolveProposal: async (proposalId: string, action: "approve" | "reject") => {
            const { service, loadProposals, loadPages } = get();
        ...
        setupEventListeners: () => {
            ...
            // Use event router to listen for wiki events
            // @ts-ignore
            const eventRouter = window.__EVENT_ROUTER__;
            ...
            // Register to event router if available, otherwise use window event system
            if (eventRouter && eventRouter.onWikiEvent) {
                eventRouter.onWikiEvent(wikiEventHandler);
            } else {
                // Fallback: use window event system
                // @ts-ignore
                if (window.__WIKI_EVENT_HANDLERS__) {
                    // @ts-ignore
                    window.__WIKI_EVENT_HANDLERS__.push(wikiEventHandler);
                } else {
                    // @ts-ignore
                    window.__WIKI_EVENT_HANDLERS__ = [wikiEventHandler];
                }
            }
        ...
        cleanupEventListeners: () => {
            ...
            if (eventHandler) {
                ...
                if (eventRouter && eventRouter.offWikiEvent) {
                    eventRouter.offWikiEvent(eventHandler);
                } else {
                    // Fallback: remove from window event handlers
                    // @ts-ignore
                    if (window.__WIKI_EVENT_HANDLERS__) {
                        // @ts-ignore
                        const handlers = window.__WIKI_EVENT_HANDLERS__;
