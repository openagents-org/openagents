      ...
      return {
        ...
        legacyAttachment: threadMsg.attachment_file_id
          ? {
              ...
              size:
                typeof threadMsg.attachment_size === "string"
        ...
        >
          ...
          {messageProps.reactions &&
            Object.keys(getValidReactions(messageProps.reactions)).length >
              ...
              <div className="flex flex-wrap gap-1 mt-2">
                {Object.entries(
                  getValidReactions(messageProps.reactions)
                ).map(([type, count]) => (
                  <div
                    key={type}
                    ...
                    onClick={(event) =>
                      handleReaction(messageId, type, messageProps.reactions, event, "add")
                    }
                  >
          ...
          >
            {!isDMChat && onReply && (
              <button
                ...
                onClick={() =>
                  onReply(messageId, messageProps.content, messageProps.senderId)
                }
                title="Reply"
              >
                ↩️
          ...
          {showReactionPicker === messageId && (
            <div
              ...
              style={{
                transform:
                  level === 0 && messageIndex === 0 ? "translateY(50px)" : "none",
              }}
            >
    ...
    return (
      <div key={message.id} className="mb-1 relative">
                ...
                {Object.entries(getValidReactions(message.reactions)).map(
                  ([type, count]) => (
                    <div
                      ...
                      className="flex items-center gap-1 px-2 py-0.5 rounded-full text-xs cursor-pointer transition-all duration-150 border bg-slate-100 border-slate-200 hover:bg-slate-200 hover:border-slate-300 dark:bg-slate-600 dark:border-slate-500 dark:text-gray-200 dark:hover:bg-slate-500 dark:hover:border-slate-400"
                      onClick={(event) =>
                        handleReaction(message.id, type, message.reactions, event, "add")
                      }
                    >
                      <span>{getReactionEmoji(type)}</span>
                      <span>{count}</span>
        ...
        {hasChildren && !isCollapsed && level < maxThreadDepth && (
          ...
          >
            {children?.map((child) =>
              renderUnifiedMessage(child.message, level + 1, child.children, undefined)
            )}
          </div>
        )}
      </div>
