...
export const detectLocalNetwork = async (): Promise<NetworkConnection | null> => {
  const commonPorts = [8700, 8571, 8570];

  for (const httpPort of commonPorts) {
    try {
      const response = await networkFetch('localhost', httpPort, '/api/health', {
        method: 'GET',
        timeout: 3000,
      });
...
export const fetchNetworkHealth = async (
  ...
  try {
    console.log(
      `Fetching health information from ${connection.host}:${connection.port}`
    );
