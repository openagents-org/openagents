...
const WikiPageList: React.FC = () => {
  ...
  // Create service adapter from connector
  ...
  useEffect(() => {
    console.log("WikiPageList: Service from context:", {
      hasService: !!service,
      isConnected,
      serviceType: service ? typeof service : 'null',
    });
    
    if (service) {
      console.log("WikiPageList: Setting service to store");
      setService(service);
    } else {
      console.warn("WikiPageList: Service is null, cannot set to store");
    }
  }, [service, isConnected, setService]);
  ...
  }, [service, isConnected, loadPages, loadProposals]);
  ...
  }, [service, setupEventListeners, cleanupEventListeners]);
