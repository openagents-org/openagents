...
class SharedDocumentAgentAdapter(BaseModAdapter):
    ...
    async def process_incoming_event(self, event: Event) -> Optional[Event]:
        """Process incoming events from the network.
        
        Args:
            event: The event to process
            
        Returns:
            Optional[Event]: The processed event, or None if consumed
        """
        try:
            ...
            if event_name == "document.history":
                await self._handle_history_message(event)
                return None
            elif event_name == "document.identity":
                await self._handle_identity_message(event)
                return None
            elif event_name == "document.user_info":
                await self._handle_user_info_message(event)
                return None
            elif event_name == "document.user_cursor":
                await self._handle_user_cursor_message(event)
                return None
            elif event_name == "document.language":
                await self._handle_language_message(event)
                return None
            elif event_name == "document.error":
                await self._handle_error_message(event)
                return None
            ...
            logger.error(f"Error processing incoming event {event.event_name}: {e}")
            return event
    ...
    def register_document_handler(self, handler_name: str, handler: DocumentHandler) -> None:
        ...
        """
        logger.debug(f"Registering document handler: {handler_name}")
        self.document_handlers[handler_name] = handler
        logger.debug(f"Total registered handlers: {len(self.document_handlers)}")
    ...
    async def request_user_identity(self) -> Dict[str, Any]:
        ...
        try:
            ...
            identity_message = IdentityDocumentMessage(
                source_id=self.agent_id,
                payload={}
            )

            # Send to network
            await self.connector.send_event(identity_message)
    ...
    async def _handle_identity_message(self, message: Event):
        ...
        try:
            ...
            # Call registered handlers
            logger.debug(f"Calling {len(self.document_handlers)} registered document handlers")
            for handler_name, handler in self.document_handlers.items():
                try:
                    event_data = {
                    ...
                    logger.debug(f"Calling document handler: {handler_name} with data: {event_data}")
                    handler(event_data)
                    logger.debug(f"Successfully called document handler: {handler_name}")
                except Exception as e:
                    logger.error(f"Error in document handler {handler_name}: {e}")
                    logger.exception(f"Full traceback for handler {handler_name}:")
