...
<<<<<<< HEAD
    ...
    def _load_workspace_data(self):
        ...
        try:
            ...
            if agent_mappings_file.exists():
                ...
                logger.info(f"Loaded {len(self.agent_workspaces)} agent workspace mappings from storage")
    ...
    def _save_workspace_data(self):
        ...
        try:
            ...
            logger.info(f"Saved workspace data: {len(self.workspaces)} workspaces, {len(self.agent_workspaces)} agent mappings")
            
        except Exception as e:
            logger.error(f"Failed to save workspace data: {e}")
    
    def create_workspace(self, workspace_id: str, workspace_data: Dict[str, Any]) -> bool:
    ...
    def assign_agent_to_workspace(self, agent_id: str, workspace_id: str) -> bool:
        ...
        """
        if workspace_id not in self.workspaces:
            logger.warning(f"Cannot assign agent {agent_id} to non-existent workspace {workspace_id}")
            return False
