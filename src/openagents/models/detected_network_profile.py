...
class DetectedAgentInfo(BaseModel):
    ...
    transport_type: TransportType = Field(..., description="Transport type used by the agent")
...
class DetectedModInfo(BaseModel):
    ...
    config: Dict[str, Any] = Field(default_factory=dict, description="Mod configuration")
...
class DetectedTransportInfo(BaseModel):
    ...
    config: Dict[str, Any] = Field(default_factory=dict, description="Transport configuration")
...
class DetectedNetworkProfile(BaseModel):
    ...
    is_running: bool = Field(..., description="Whether the network is currently running")
    ...
    topology_mode: str = Field(..., description="Network topology mode (centralized/decentralized)")
    transports: List[DetectedTransportInfo] = Field(
        default_factory=list,
        description="Available transport configurations"
    )
    manifest_transport: Optional[str] = Field(None, description="Transport used for manifests")
    recommended_transport: Optional[str] = Field(None, description="Recommended transport type")
    max_connections: int = Field(100, description="Maximum number of connections allowed")
    ...
    mods: List[DetectedModInfo] = Field(
        default_factory=list,
        description="Information about loaded mods"
    )
    ...
    def from_network_stats(cls, stats: Dict[str, Any]) -> "DetectedNetworkProfile":
        ...
        for transport_data in stats.get("transports", []):
            if isinstance(transport_data, dict):
                transports.append(DetectedTransportInfo(
    ...
    def to_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "agents": {
                ...
                "online": self.online_agent_count
            },
            "mods": {
                "total": len(self.mods),
                "enabled": self.enabled_mod_count
            },
